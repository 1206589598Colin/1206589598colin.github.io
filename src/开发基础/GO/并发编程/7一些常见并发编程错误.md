...
你觉得此程序会输出什么？ `123` 还是 `789` ？ 事实上，它的输出是和具体使用的编译器相关的。 对于标准编译器1.16版本来说，它很可能输出 `123` 。 但是从理论上说，它输出 `789` 或者另外一个预想不到的值也是有可能的。
...
接收者协程可能会永久阻塞的原因是如果5个尝试发送操作都发生在接收操作 `<-c` 准备好之前，亦即5个个尝试发送操作都失败了，则接收者协程将永远无值可接收（从而将处于永久阻塞状态）。
